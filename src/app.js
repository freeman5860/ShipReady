...
import { getAudit, getShare, getShareByAudit, saveAudit, saveShare, storageMode } from "./store.js";
...
export async function handleRequest(req, res, options = {}) {
  try {
    ...
    if (req.method === "GET" && pathname.startsWith("/r/")) {
      return renderPublicReport(res, pathname.split("/").pop());
    }
...
async function handleApi(req, res, pathname) {
  if (req.method === "GET" && pathname === "/api/health") {
    return sendJson(res, 200, { ok: true, storage: storageMode() });
  }
  ...
  if (req.method === "POST" && pathname === "/api/brief") {
    const body = await readJson(req);
    const record = await requireAudit(body.audit_id);
  ...
  if (req.method === "POST" && pathname === "/api/brief/follow-up") {
    const body = await readJson(req);
    const record = await requireAudit(body.audit_id);
  ...
  if (req.method === "POST" && pathname === "/api/share") {
    ...
    const existing = await getShareByAudit(record.id);
    const share =
      existing ||
      {
        id: createShareId(),
  ...
  if (req.method === "GET" && pathname.startsWith("/api/report/")) {
    ...
    const record = await requireAudit(id);
    return sendJson(res, 200, serializeRecord(record));
  }
...
async function requireAudit(id) {
...
async function renderPublicReport(res, shareId) {
  const share = await getShare(shareId);
  if (!share) return sendText(res, 404, "Report not found");
  const record = await getAudit(share.auditId);
...
function normalizePathname(requestUrl) {
  if (requestUrl.pathname === "/api/server" && requestUrl.searchParams.has("path")) {
    return requestUrl.searchParams.get("path");
  }
  if (requestUrl.pathname === "/api/server" && requestUrl.searchParams.has("share")) {
    return `/r/${requestUrl.searchParams.get("share")}`;
  }
